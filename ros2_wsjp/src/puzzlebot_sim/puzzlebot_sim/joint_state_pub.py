...
class FramePublisher(Node):
    ...
    def __init__(self):
        ...
        self.br_footprint2lidar = TransformBroadcaster(self)
    ...
    def laser_callback(self, msg):
        # No longer using the laser timestamp to avoid TF_OLD_DATA
        pass
    ...
    def timer_callback(self):
        ...
        tf_odom_footprint.header.stamp = now
        ...
        # --- base_footprint -> lidar_base_link ---
        tf_lidar = TransformStamped()
        tf_lidar.header.stamp = now
        tf_lidar.header.frame_id = 'odom'
        tf_lidar.child_frame_id = 'laser'
        tf_lidar.transform.translation.x = 0.0
        tf_lidar.transform.translation.y = 0.0
        tf_lidar.transform.translation.z = 0.0
        q_lidar = transforms3d.euler.euler2quat(0, 0, 0)
        tf_lidar.transform.rotation.x = q_lidar[1]
        tf_lidar.transform.rotation.y = q_lidar[2]
        tf_lidar.transform.rotation.z = q_lidar[3]
        tf_lidar.transform.rotation.w = q_lidar[0]
        self.br_footprint2lidar.sendTransform(tf_lidar)
